...
isAfterDate(date, isAfter) {
  ...
  getCurChodesh() {
	const itemsAfterNow = this.cal.filter(item => this.isAfterToday(new Date(item["date"])));
	...
	const firstCandleLightingDate = new Date(candleLightingItemsAfterNow[0]["date"]);
     	const firstHavdallahDate = new Date(havdallahItemsAfterNow[0]["date"]);

	var memo = havdallahItemsAfterNow[0]["memo"];

	var yesterday = new Date(this.today.getTime());
        yesterday.setDate(yesterday.getDate() - 1);
	const isSecondCandleLighting = this.cal.some(item => {
		var comp = new Date(item["date"]);
		comp.setDate(comp.getDate() + 1);
		const ret = (item["category"] === "candles"
			&& comp.getFullYear() === firstCandleLightingDate.getFullYear()
			&& comp.getDate() === firstCandleLightingDate.getDate()
			&& comp.getMonth() === firstCandleLightingDate.getMonth());
		if (ret) {
			memo = candleLightingItemsAfterNow[0]["memo"];
		}
	});


	if (this.isAfterDate(firstHavdallahDate, firstCandleLightingDate) && !isSecondCandleLighting) {
		return "";
	}
  ...
  checkChodesh(chodeshKey) {
	...
	if (ret) { this.log.info("is " + chodeshKey); }
        return ret;
  }
 ...
 checkHoliday(key) {
	...
	if (ret) { this.log.info("is " + key + "(holiday)"); }
	return ret;
 }
  ...
  checkTodayBasic(key) {
	...
	if (ret) { this.log.info("is " + key + "(basic)"); }
        return ret;
  }
  ...
  isThreeWeeks() {
	...
	if (ret) { this.log.info("is three weeks"); }
	return ret;
  }
  ...
  isMourning() { return this.isOmer() || this.isThreeWeeks();}
  ...
  isChanukah() {
	...
	if (ret) { this.log.info("is chanukah"); }
	return ret;
  }
}
